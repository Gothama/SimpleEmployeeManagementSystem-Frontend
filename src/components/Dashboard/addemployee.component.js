...
export default class AddEmployee extends Component {
  ...
  onSubmit = () => {
    ...
    axios
      .post("http://localhost:9090/employee/addemployee", {
        nic: this.state.nic,
        name: this.state.name,
        position: this.state.position,
        telephoneNumber: this.state.telephonenumber,
      ...
      .then((res) => {
        ...
        if (res.data !== null) {
          this.successfulmessage("Employee Added Successfull");
        } else {
          {
  ...
  render() {
    const { nic, name, salary,position,telephonenumber } = this.state;
    return (
      <div>
        <div
          style={{
